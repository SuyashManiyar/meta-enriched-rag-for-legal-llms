...
# Australian Legal Test Data - ACTIVE (Using Meta-Recursive)
INPUT_TEST_JSON = "australian_legal_data/RAG_ground_truth_corrected.json" # RAG Ground Truth JSON (146 QA pairs)
QUERY_ID_JSON = os.path.join(OUTPUT_DIR, "australian_legal_w_query_ids.json") # Query IDs file in output directory
METADATA_JSON = "RAG_pipeline/embeddings_with_span/australlian_legal_faiss_emb_meta_recursive.json" # Meta-Recursive
FAISS_INDEX = "RAG_pipeline/embeddings_with_span/australian_legal_faiss_emd_meta_recursive.bin" # Meta-Recursive FAISS index
OUTPUT_RETRIEVAL_JSON_DENSE_ONLY = os.path.join(OUTPUT_DIR, "results_meta_recursive/australian_legal_retrieval_results_meta_recursive_dense_only.json") # Output with meta-recursive dense-only
OUTPUT_RETRIEVAL_JSON_DENSE_SPARSE = os.path.join(OUTPUT_DIR, "results_meta_recursive/australian_legal_retrieval_results_meta_recursive_dense_sparse.json") # Output with meta-recursive dense+sparse 
...
def assign_query_ids(input_json_path, output_json_path):
    ...
    if "tests" in data:
        ...
        # RAG Ground Truth format (direct dict of QA pairs)
        out = {}
        qid = 1
        
        for qa_id, qa_data in tqdm(data.items()):
            key = f"query_id_{qid}"
            
            # For Australian legal data, the doc_id is the key itself (001, 003, etc.)
            doc_id = qa_id
            
            out[key] = {
                "query": qa_data.get("question", ""),
                "doc_id": doc_id,
                "original_qa_id": qa_id,
                "citation": qa_data.get('citation', ''),
                "text": qa_data.get("text", ""),  # Ground truth text
                "span": qa_data.get("span", []),  # Ground truth span
                "document_path": qa_data.get("document_path", "")
            }
            qid += 1
...
# ============================================================
# HELPER: Extract doc_id from descriptive chunk_id
# ============================================================
def extract_doc_id_from_chunkid(chunk_id: str) -> str:
    """
    Extract document ID from descriptive chunk ID.
    Format: australian_legal_texts_001_judgments.fedcourt.gov.au_...
    Returns: 001
    """
    if not chunk_id or not chunk_id.startswith("australian_legal_texts_"):
        return None
    ...
    # The doc_id is the 4th part (index 3): australian_legal_texts_001_...
    return parts[3]
...
def build_output_json_dense_sparse(
    ...
    for qid, qinfo in query_data.items():
        ...
        for sc, meta in fused_results:
            chunk_id = meta["chunk_id"]
            retrieved_doc_id = extract_doc_id_from_chunkid(chunk_id)
...
if __name__ == "__main__":
    print(f"🚀 Starting Australian Legal RAG Inference (Meta-Recursive Chunking)...")
    print(f"📂 Output directory: {OUTPUT_DIR}")
    print(f"📄 Input RAG ground truth: {INPUT_TEST_JSON}")
    print(f"📊 Expected QA pairs: 146")
    print(f"🔍 Using FAISS index: {FAISS_INDEX}")
    print(f"📋 Using metadata: {METADATA_JSON}")
    
    # Step 1: Assign query IDs and save to australian_legal_data folder
    print(f"\n📝 Step 1: Checking query IDs...")
    if not os.path.exists(QUERY_ID_JSON):
        assign_query_ids(INPUT_TEST_JSON, QUERY_ID_JSON)
        print(f"✅ Query IDs saved to: {QUERY_ID_JSON}")
    else:
        print(f"✅ Query IDs already exist: {QUERY_ID_JSON}")

    # Step 2: Build dense-only retrieval results
    print(f"\n🔍 Step 2: Running dense-only retrieval...")
    build_output_json(
        query_id_json=QUERY_ID_JSON,
        meta_path=METADATA_JSON,
        faiss_path=FAISS_INDEX,
        output_json=OUTPUT_RETRIEVAL_JSON_DENSE_ONLY
    )
    print(f"✅ Dense-only results saved to: {OUTPUT_RETRIEVAL_JSON_DENSE_ONLY}")
    
    # Step 3: Build dense+sparse retrieval results
    print(f"\n🔍 Step 3: Running dense+sparse retrieval...")
    print(f"   - Dense: FAISS embeddings (GTE-large)")
    print(f"   - Sparse: BM25 over chunk text")
    print(f"   - Fusion: α=0.8 (80% dense, 20% sparse)")
    build_output_json_dense_sparse(
        query_id_json=QUERY_ID_JSON,
        meta_path=METADATA_JSON,
        faiss_path=FAISS_INDEX,
        output_json=OUTPUT_RETRIEVAL_JSON_DENSE_SPARSE,
        topk=64,
        alpha=0.8
    )
    print(f"✅ Dense+sparse results saved to: {OUTPUT_RETRIEVAL_JSON_DENSE_SPARSE}")
    print(f"\n🎉 Australian Legal RAG Meta-Recursive Inference completed!")
